...
class UserValidator(Validator):
    ...
    def validate(entity):
        ...
        if not entity.blood_group_id in (1, 2, 3, 4, 5, 6, 7, 8) and entity.profile_setup is True:
            errors.append("Blood group is invalid!")

        if not re.match(r'((?:\+|00)[17](?: |\-)?|(?:\+|40)[1-9]\d{0,2}'
                        r'(?: |\-)?|(?:\+|00)1\-\d{3}(?: |\-)?)?(0\d|\([0-9]{3}\)|'
                        r'[1-9]{0,3})(?:((?: |\-)[0-9]{2}){4}|((?:[0-9]{2}){4})|((?: |\-)'
                        r'[0-9]{3}(?: |\-)[0-9]{4})|([0-9]{7}))', entity.phone_number) and entity.profile_setup is True:
